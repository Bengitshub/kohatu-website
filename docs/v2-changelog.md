...
## Final pre-pitch round (21 Jul 2026, commit de364be)
Per Ben's final implementation brief: Torque & Trails rewritten (Codgers Round One "previous format" per brief — flagged unverified by Webhero; genuine e-MTB photos; all "at Kohatu Park" claims removed) · FYP price → "Price to be confirmed" everywhere incl. JSON-LD, "Sweep support", provisional-inclusions note, entry-options explainer under timeline · Deep in the Sounds → "Contact Dave for availability" status w/ mailto CTA (post-26-Jul flip = one status change in events.json) · festival cards "Adventure and road-bike riders" + split Adventure/Road programme sections w/ separate NZ$ pricing · park NZ$ prices + brief's exact bring-answer (rego/WoF/insurance claim removed) · homepage/about/safety claim language per brief · site-wide meta robots noindex,nofollow (PREVIEW flag in build_v2.py; flip at launch) · /proposal/ offer: NZ$3,950 fixed, 2 revision rounds, domain+deploy, forms+mailing, analytics+Search Console, redirects, 30-day defect support, optional NZ$129/mo care plan; ticketing provider genericised. QA: banned-words scan clean (no Humanitix/AI/dev-process references), 31 pages, 0 broken refs, JSON-LD valid, no mobile overflow (live 375px viewport), forms name+email only, dialogs open/close, sticky CTA works.
